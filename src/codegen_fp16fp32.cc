...
int main(int argc, const char* argv[]) {
  ...
  for (auto& d_type : types_to_gen) {
    ...
    for (auto s : isa) {
      ...
      string intrinsic_fun_prefix = s.iset == ISA::isaType::avx512 ? "__m512_" : "__m256_";
      ...
      for (unsigned k = 0; k < ukernel_shape.size(); k++) {
        ...
        auto const A_load_mult = [&](int r, mult_type m_type) {
          if (prefetch_a_len && ((4 * r) % cache_line_size == 0)) {
            addi(
                srcfile,
                "_mm_prefetch((char*)r9, " + to_string(prefetch_a_len) + ")",
                "prefetcht0 [r9 + " + to_string(prefetch_a_len) + "]",
                fixedC);
          }
          string mul = m_type == mult_type::mul ? "mul_ps" : "vfmadd231ps";
          if(s.iset == ISA::isaType::avx512) {
            addi(srcfile,
                 "    auto loaded_xmm = _mm_broadcast_ss((float*)((char*)r9 + " + to_string(4 * r)
                     + ")); \n    auto " + vAtmp + " = " + intrinsic_fun_prefix
                     + "broadcastss_ps((__m128)loaded_xmm));",
                 "vbroadcastss " + vAtmp + ",DWORD PTR [r9+" + to_string(4 * r) + "]");
          } else {
            addi(srcfile,
                 "    auto " + vAtmp + " = " + intrinsic_fun_prefix + "broadcast_ss((char*)r9 + " + to_string(4 * r) + "));",
                 "vbroadcastss " + vAtmp + ",DWORD PTR [r9+" + to_string(4 * r) + "]");
          }
          for (int c = 0; c < vCtile[0].size(); c++) {
            addi(
                srcfile,
                "    auto " + vCtile[r][c] + " = " + intrinsic_fun_prefix + mul + "(" + vBcol[c] + "," + vAtmp + "));",
        ...
        auto const B_load = [&](int c, const string& vBcol, int prefetch_len) {
          if (d_type.first == DataType::Float32) {
            addi(
                srcfile,
                 "    auto " + vBcol + " = " + intrinsic_fun_prefix + "loadu_ps((char*)r10 + "
            ...
            if(s.iset == ISA::isaType::avx512) {
              addi(
                  srcfile,
                   "    auto fp16mem" + to_string(16 * c) + " = _mm256_load_si256((char*)r10 + "
                       + to_string(vec_len_in_bytes / 2 * c) + "));\n" + "    auto " + vBcol
                       + " = _mm512_cvtph_ps(fp16mem" + to_string(16 * c) + ");",
                   "vcvtph2ps " + vBcol + "," +
              ...
              addi(srcfile,
                   "    auto fp16mem" + to_string(16 * c) + " = _mm128_load_si128((char*)r10 + "
                       + to_string(vec_len_in_bytes / 2 * c) + "));\n" + "    auto " + vBcol
                       + " = _mm256_cvtph_ps(fp16mem" + to_string(16 * c) + ");",
                   "vcvtph2ps " + vBcol + "," + (s.iset == ISA::isaType::avx512 ? "YMM" : "XMM")
                       + "WORD PTR [r10 + " + to_string(vec_len_in_bytes / 2 * c) + "]");
            }
          }
          if (prefetch_len && ((vec_len_in_bytes * c) % cache_line_size == 0)) {
            addi(
                srcfile,
                 "_mm_prefetch((char*)r10, " + to_string(prefetch_len) + ")",
                 "prefetcht0 [r10 + " +
        ...
        auto const C_prefetch = [&](int r) {
          for (auto c = 0; prefetch_c_len && (c < vCtile[r].size()); c++) {
            if ((vec_len_in_bytes * c) % cache_line_size == 0) {
              addi(
                  srcfile,
                   "_mm_prefetch((char*)r12, " + to_string(c * cache_line_size + prefetch_c_len)
                       + ")",
                   "prefetcht1 [r12 + "
                       +
                      to_string(
          ...
          for (auto c = 0; c < vCtile[r].size(); ++c) {
            switch (s.iset) {
              ...
              case ISA::isaType::avx512_256:
                ...
                if(s.iset == ISA::isaType::avx512) {
                  addi(srcfile,
                       "    auto loaded_xmm = _mm_broadcast_ss((float*)((char*)r12 + "
                           + to_string(vec_len_in_bytes * c) + ")); \n    auto " + vAtmp + " = "
                           + intrinsic_fun_prefix + "broadcastss_ps((__m128)loaded_xmm));",
                       "vmulps " + vCtile[r][c] + ", " + r_spare + ", " +
                        "[r12 + " + to_string(vec_len_in_bytes * c) + "]");
                } else {
                  addi(srcfile,
                       "    auto " + vAtmp + " = " + intrinsic_fun_prefix
                           + "broadcast_ss((char*)r12 + " + to_string(vec_len_in_bytes * c) + "));",
                       "vmulps " + vCtile[r][c] + ", " + r_spare + ", " +
                        "[r12 + " + to_string(vec_len_in_bytes * c) + "]");
                }
                addi(
                    srcfile,
                     "    auto " + vCtile[r][c] + " = " + intrinsic_fun_prefix + "mul_ps(" + r_spare
                         + "," + vAtmp + "));",
                     "vmulps " + vCtile[r][c] + ", " + r_spare + ", " +
                        "[r12 + " + to_string(vec_len_in_bytes * c) + "]",
                    fixedC);
        ...
        srcfile << "      // k\n";
        addi(srcfile, "uint64_t    r8  = *(uint64_t   *)((char*)r14 + 0 );", "mov r8, [r14 + 0]");        // Assuming k >= 1
        addi(srcfile, "r8--", "dec r8");
        srcfile << "      // A\n";
        ...
        srcfile << "      // C\n";
        addi(srcfile, "float*      r12 = *(float*     *)((char*)r14 + 32);", "mov r12, [r14 + 32]");
        srcfile << "      // ldc\n";
        addi(srcfile, "uint64_t    r13 = *(uint64_t   *)((char*)r14 + 40);", "mov r13, [r14 + 40]");
        srcfile << "      // b_block_cols\n";
        addi(srcfile, "uint64_t    rdi = *(uint64_t   *)((char*)r14 + 48);", "mov rdi, [r14 + 48]");
        srcfile << "      // b_block_size\n";
        addi(srcfile, "uint64_t    rsi = *(uint64_t   *)((char*)r14 + 56);", "mov rsi, [r14 + 56]");
        srcfile << "\n";
        ...
        addi(srcfile, "for (; rbx < rdi; ++rbx) {", "inc rbx; cmp rbx, rdi; jl " + label_outer);
        addi(srcfile, "// ", label_outer + ":");
        addi(srcfile, "  uint64_t r14_i = r8;", "mov r14, r8");
        ...
        if(s.iset == ISA::isaType::avx512) {
          addi(
              srcfile,
               "    auto loaded_xmm = _mm_broadcast_ss((float*)((char*)r15)); \n    auto " 
                   + r_spare + " = "
                   + intrinsic_fun_prefix
                   + "broadcastss_ps((__m128)loaded_xmm));",
               "vbroadcastss " + r_spare + string(",DWORD PTR [r15]"),
              fixedC);
        } else {
              addi(srcfile,
                   "    auto " + r_spare + " = " + intrinsic_fun_prefix + "broadcast_ss((char*)r15));",
                  "vbroadcastss " + r_spare + string(",DWORD PTR [r15]"),
                  fixedC);
        }
        ...
        addi(
            srcfile,
             "  rcx = (float*)((char*)rcx + " + to_string(vec_len_in_bytes * ukernel_shape[k][1])
                 + ");",
        ...
        addi(srcfile, "}", "inc rbx; cmp rbx, rdi; jl " + label_outer);
...
//}
//assert(last_free_vecreg <= num_vec_regs);
//string r_spare = vec_reg_prefix + to_string(num_vec_regs - (s.iset == ISA::isaType::avx ? 2 : 1));
//
//auto const A_load_mult = [&](int r, mult_type m_type) {
//  if(prefetch_a_len && ((4 * r) % cache_line_size == 0)) {
//    addi(srcfile, "prefetcht0 [r9 + " + to_string(prefetch_a_len) + "]", fixedC);
//  }
//  string mul = m_type == mult_type::mul ? "vmulps" : "vfmadd231ps";
//  addi(srcfile, "vbroadcastss " + vAtmp + ",DWORD PTR [r9+" + to_string(4 * r) + "]");
//  for(int c = 0; c < vCtile[0].size(); c++) {
//    addi(srcfile, mul + " " + vCtile[r][c] + "," + vBcol[c] + "," + vAtmp);
//  }
//};
//
//// Generate Loads from Matrix B
...
//  if(d_type.first == DataType::Float32) {
//    addi(srcfile,
//         "vmovups " + vBcol + "," + (s.iset == ISA::isaType::avx512 ? "ZMM" : "YMM")
//             + "WORD PTR [r10 + " + to_string(vec_len_in_bytes * c) + "]");
//  } else if(d_type.first == DataType::Float16) {
//    addi(srcfile,
//         "vcvtph2ps " + vBcol + "," + (s.iset == ISA::isaType::avx512 ? "YMM" : "XMM")
...
//  if(prefetch_len && ((vec_len_in_bytes * c) % cache_line_size == 0)) {
//    addi(srcfile,
//         "prefetcht0 [r10 + " + to_string(vec_len_in_bytes * c + prefetch_len) + "]",
//         fixedC);
//  }
//};
//
...
//      case ISA::isaType::avx512:
//      case ISA::isaType::avx512_256:
//        if(prefetch_c_len && ((vec_len_in_bytes * c) % cache_line_size == 0)) {
//          addi(srcfile,
//               "prefetcht1 [r12 + "
//                   + to_string(
//                         /*vec_len_in_bytes * ukernel_shape[k][1] +*/
...
//
//string r_spare_cmp = "xmm" + to_string(num_vec_regs - (s.iset == ISA::isaType::avx ? 2 : 1));
//
//addi(srcfile, "vbroadcastss " + r_spare + string(",DWORD PTR [r15]"), fixedC);
//// Generate first iteration which loads values from C  and interleavs
//// With loads from B and multiplication
//for(auto c = 0; c < vCtile[0].size(); ++c) {
//  B_load(c, vBcol[c], prefetch_b_len);
...
//  for(auto c = 0; c < vCtile[r].size(); c++) {
//    addi(srcfile,
//         "vmovups " + vec_reg_prefix + "word PTR [r12 + " + to_string(vec_len_in_bytes * c) + "], "
//             + vCtile[r][c],
//         fixedC);
//  }
//}
...
//srcfile << "\n      // next outer iteration\n";
//// C
//addi(srcfile, "add rcx, " + to_string(vec_len_in_bytes * ukernel_shape[k][1]), fixedC);
