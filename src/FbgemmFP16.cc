...
constexpr std::array<knl_ptr, 15> kernel_avx2 = {nullptr,
...
constexpr std::array<knl_ptr, 15> kernel_avx512_256 = {nullptr,
                                                       gemmkernel_1x2_Avx2_fp16_fA0fB0fC0,
                                                       gemmkernel_2x2_Avx2_fp16_fA0fB0fC0,
                                                       gemmkernel_3x2_Avx2_fp16_fA0fB0fC0,
                                                       gemmkernel_4x2_Avx2_fp16_fA0fB0fC0,
...
constexpr std::array<knl_ptr, 15> kernel_avx512 = {nullptr,
...
// clang-format off
  ...
  namespace {
  void ref_kernel(int kernel_nrows,
                  GemmParams* gp,
                  const float* C_base,
                  int m_total,
                  int n_total,
      ...
      for(int k = 0; k < gp->k; ++k) {
        for(int i = 0; i < kernel_nrows; ++i) {
          ...
          for(int j = 0; j < block_col_size; ++j) {
            float* C_ptr = gp->C + i * (gp->ldc / sizeof(float)) + jb * block_col_size + j;
            assert(C_ptr < C_base + m_total * n_total);
            float b = cpu_half2float(gp->B[(jb * gp->k + k) * block_col_size + j]);
            if(gp->accum) {
  ...
  void cblas_gemm_compute(const matrix_op_t transa,
    ...
    bool use_avx512
        = isZmm(isa)
          && (Bp.blockColSize()
              == simd_info<inst_set_t::avx512>::WIDTH_32BIT_ELEMS * Bp.kernelNumColBlocks());
    ...
    const auto& kernels = use_avx512 ? KernelInfo::kernel_avx512
        ...
        for(auto c = 0; c < 2; c++) {
          ...
          for(auto m2 = m_start; m2 < m_end; m2 += kernel_nrows) {
            ...
            if((n % Bp.blockColSize()) == 0) {
              int jb_begin, jb_end;
              fbgemmPartition1D(thread_id, num_threads, gp.b_block_cols, jb_begin, jb_end);
              ...
              if(thread_id == num_threads - 1) {
                ...
                // registers.
                float c_tmp[14 * 32] = {0};
                assert(sizeof(c_tmp) / sizeof(c_tmp[0]) >= kernel_nrows * Bp.blockColSize());
                ...
                for(int i = 0; i < kernel_nrows; i++) {
                  ...
                  for(int j = last_blk_col; j < n; j++) {
                    assert(i * Bp.blockColSize() + (j - last_blk_col)
                           < sizeof(c_tmp) / sizeof(c_tmp[0]));
                    if(beta_ == 0.f) {
                      C[(m2 + i) * ldc + j] = c_tmp[i * Bp.blockColSize() + (j - last_blk_col)];
                    } else {
